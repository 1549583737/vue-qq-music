    ...
    export default {
        ...
        methods: {
            ...
            leave(el, done) {
                ...
                const {x,y,scale} = this._getPosAndScale()
                this.$refs.cdWrapper.style[transform] = `translate3d(${x}px, ${y}px, 0) scale()`
            },
            afterLeave() {
            ...
            _getPosAndScale() {
              ...
              const paddingLeft = 40;   //小圆圆心距离左面距离40px
              const paddingBottom = 30; //小圆圆心距离底部30px
              const paddingTop = 80;    //大圆顶部距离页面顶部80px
              const width = window.innerWidth * 0.8;  //大圆的宽度
              const scale = targetWidth / width;  //小圆相对于大圆的缩放比例
