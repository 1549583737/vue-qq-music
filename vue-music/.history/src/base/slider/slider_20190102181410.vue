...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        mounted() {
            ...
            //切换浏览器窗口模式改变浏览器窗口大小重新计算轮播图
            window.addEventListener('resize', ()=> {
                if(!this.slider){   //如果没有初始化直接返回
                    return 
                }
        ...
        methods: {
            ...
            _initSlider() {
                ...
                this.slider.on("scrollEnd", ()=>{
                    ...
                    if(this.loop){
                        pageIndex -= 1; //去掉为了循环播放而多的第一页 创建new Scroll时自动克隆的两个DOM
                    }
                    this.currentPageIndex = pageIndex;

                    if(this.autoPlay && !isResize){
                        clearTimeout(this.timer);   //清除定时器 如果手动拖动轮播图会有两个定时器 奇怪现象一前一后
                        this._play();   //自动
                    }
                })
            },
